...
public class CooldownManager implements Tickable {
    ...
    public void tick(int globalTick) {
        ...
        for (UUID uuid : map.keySet()) {
            Player player = CruiseSkyblock.plugin.getServer().getPlayer(uuid);
            if (player == null) {
                map.remove(uuid);
                continue;
            }
    ...
    public void sendCooldownMessage(Player player, CustomItem item) {
        ...
        if (item != null && holder != null && holder.contains(item)) {
            ...
            if (tick <= finishTime - 20) {
                ...
                TranslatableComponent component = new TranslatableComponent("gui.csb.cooldown", sec);
                ...
                TranslatableComponent component = new TranslatableComponent("gui.csb.cooldown.available");
                component.setColor(ChatColor.GRAY);
                player.spigot().sendMessage(ChatMessageType.ACTION_BAR, component);
            }
        }
