...
public class VariableSizedSidebar implements Sidebar {
    ...
    public VariableSizedSidebar(Player player) {
        this.player = player;
        teams = new ArrayList<>();
        if (CruiseSkyblock.scoreboardManager != null) {
            setScoreboard(CruiseSkyblock.scoreboardManager);
        }
    }
    ...
    public void setScoreboard(ScoreboardManager manager) {
        if (scoreboard == null && manager != null) {
            ...
            String objectiveName = CruiseSkyblock.SHORT_NAME + "_sidebar_" + player.getName();
